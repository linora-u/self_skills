...
当特性分支的开发工作完成、代码提交并测试通过后，你需要将它合并回主分支（如 `main`）并清理当前工作区。

推荐使用 `wt merge` 指令（这会一步到位完成多个操作）。

### 使用方法

首先，进入**已完成开发的特性分支工作区目录**，并确保所有更改已提交：

```bash
wt merge <目标分支>   # 我们通常使用：wt merge main
```

### 幕后执行流程

`wt merge` 是一条自动化指令，执行后会按照下述顺序工作：
1. **自动合并 (Squash & Rebase):** 将当前分支的全部提交压缩打包，并以 Rebase（变基）的方式合并给目标分支（如 `main`）。
2. **快进分支 (Fast-forward):** 让目标分支快进更新到合并后的最新节点。
3. **安全清理 (Remove Worktree):** 代码被安全合并后，它还可以帮你自动移除这个用过的临时工作区文件夹及其对应的独立分支，将环境恢复最初的清爽状态。
